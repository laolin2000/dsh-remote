...
> 测试合计 **392 项断言**：守卫 133 / 二维码 46 / 面板（真实 DOM）73 / 插件路由 55 / 桌面工具 15 / 安装器 28 / 一键安装 31 / 隧道实测 11，
> 全绿。逐条验证记录见文末「验证记录」。
...
- **逐环节体检**：面板里的「连接体检（逐环节）」按链路顺序列出**上游 → 守卫 → 隧道 → 公网可达性 → 手机链接 → 已授权设备**，
  每步给 正常/注意/失败 + 原因 + 修法（失败项直接告诉你点哪里）；命令行同一套判断：`node guard/guard.mjs doctor`。
...
test/                测试：selftest(125) qr(46) panel.dom(68) plugin(51) bin(15) install(28) setup(31) tunnel(11)
...
- **守卫**：`test/selftest.mjs`（133 项）—— fail-closed、配对码、长期链接语义、**链接 token 优先于已有会话**、
